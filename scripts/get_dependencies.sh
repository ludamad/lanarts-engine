...
LUASOCKET="$BASE_FOLDER/$BUNDLED_DEPENDENCIES_FOLDER/lua-socket"
SAY_DOT_LUA="$BASE_FOLDER/$BUNDLED_DEPENDENCIES_FOLDER/say.lua"
ANSICOLORS_DOT_LUA="$BASE_FOLDER/$BUNDLED_DEPENDENCIES_FOLDER/ansicolors.lua"
BUSTED_DOT_LUA="$BASE_FOLDER/$BUNDLED_DEPENDENCIES_FOLDER/busted.lua"
...
# Custom socket.http and other changes:
cp -r "$LUASOCKET" lua-deps/socket
# 'Busted' dependencies:
cp "$SAY_DOT_LUA" lua-deps/
cp "$ANSICOLORS_DOT_LUA" lua-deps/
cp "$BUSTED_DOT_LUA" lua-deps/
# Sierra Lua debugger:
